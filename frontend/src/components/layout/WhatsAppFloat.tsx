 ...
 *
 * Premium pass: the flat single-color circle is now a subtle green
 * gradient with a soft colored glow (a plain `shadow-lg` read generic,
 * a tinted shadow matching the button's own color reads considered),
 * plus a small white-ringed "online" dot, the same trust cue real
 * WhatsApp Business widgets use to signal someone's actually there. The
 * pulse ring behind it now uses `animate-whatsapp-pulse` (see
 * globals.css) instead of Tailwind's default `animate-ping`, a slower,
 * softer breathing motion rather than a quick, slightly twitchy blink.
 * The whole button also settles into place a beat after the page loads
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 animate-whatsapp-float-in">
      ...
      >
        ...
        <span className="shine-hover relative w-14 h-14 rounded-full bg-gradient-to-br from-[#2CE072] to-[#1DA851] shadow-[0_8px_22px_-4px_rgba(29,168,81,0.55),0_2px_6px_rgba(35,31,32,0.18)] flex items-center justify-center transition-transform group-hover:scale-105">
        ...
        <span
          className="absolute top-0 right-0 w-3.5 h-3.5 rounded-full bg-[#25D366] border-2 border-white"
          aria-hidden="true"
        />
      </a>
    </div>
